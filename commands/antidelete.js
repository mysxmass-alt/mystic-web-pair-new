...
async function storeMessage(message) {
    try {
        ...
        if (msg.conversation) {
            ...
            const imgChunks = [];
            for await (const chunk of stream) imgChunks.push(chunk);
            await writeFile(mediaPath, Buffer.concat(imgChunks));
        } else if (msg.stickerMessage) {
            mediaType = 'sticker';
            const stream = await downloadContentFromMessage(msg.stickerMessage, 'sticker');
            mediaPath = path.join(TEMP_MEDIA_DIR, `${messageId}.webp`);
            const stckChunks = [];
            for await (const chunk of stream) stckChunks.push(chunk);
            await writeFile(mediaPath, Buffer.concat(stckChunks));
            ...
            const vidChunks = [];
            for await (const chunk of stream) vidChunks.push(chunk);
            await writeFile(mediaPath, Buffer.concat(vidChunks));
        } else if (msg.audioMessage) {
            mediaType = 'audio';
            const stream = await downloadContentFromMessage(msg.audioMessage, 'audio');
            mediaPath = path.join(TEMP_MEDIA_DIR, `${messageId}.mp3`);
            const audChunks = [];
            for await (const chunk of stream) audChunks.push(chunk);
            await writeFile(mediaPath, Buffer.concat(audChunks));
        }
